...
matched_extreme_datasets['Base 1958'] = perform_psm(
    df_extreme_raw, 'is_high_energy', psm_covariates, base_period=1958, mode='static', caliper=extreme_caliper, log_file="psm_log_extreme.txt"
)
...
print(f"Generating 12 Trajectory Graphs to '{output_pdf}'...")
...
if 'matched_datasets' in locals():
    with matplotlib.backends.backend_pdf.PdfPages(output_pdf) as pdf:
        ...
        for var_config in variables_to_plot:
            ...
            fig.text(0.5, 0.5, f"{var_config['title']}\n(All 4 Matching Methods)", ha='center', fontsize=20, fontweight='bold')
            pdf.savefig(fig)
            plt.close()
